...
public class TouchInterface : MonoBehaviour {
	...
	float tapTime = 0.25;
	float gestureTime = 0.25;
	float longTouchTime = 1;
	float swipeDistance = 30;
	...
	function Update() {
		if (Application.platform != RuntimePlatform.Android || Application.platform != RuntimePlatform.IPhonePlayer) {
			...
			RaycastHit hit;
			Vector3 lastTouchPosition = touchPosition;
	...
	Vector2 normalizedDistance(Vector2 distance) {
		Vector2 newDistance = Vector2((distance.x/Screen.width), (distance.y/Screen.height));
		return newDistance;
	}
}