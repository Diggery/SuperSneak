...
public class MapControl : MonoBehaviour {
	...
	void ConnectBases() {
		...
		ConnectPoints(lastPoint, GetEnemyBase());
	}
	...
	void SetUpConnections() {
		foreach (Transform point in points) {
			...
			ConnectPoints(point, FindPointInRange(point, 5));
			if (Random.value < 0.2f) ConnectPoints(point, FindPointInRange(point, 20));
		}
	...
	Transform FindPointInRange(Transform startPoint, float range) {
		List<Transform> setOfPoints = new List<Transform>();
		foreach (Transform point in points) {
			if (point != startPoint) {
				if (!point.GetComponent<MapDot>().IsPlayerBase() && !point.GetComponent<MapDot>().IsEnemyBase()) {
					if (!IsConnected(startPoint, point)) {
						...
						if (distance < range) setOfPoints.Add(point);
					}
				}
			}
		}
		if (setOfPoints.Count < 1) {
			if (range > 100) return null;
			return FindPointInRange(startPoint, 101);
		}
		Transform selection = setOfPoints[Random.Range(0, setOfPoints.Count)];
		return selection;
	}
